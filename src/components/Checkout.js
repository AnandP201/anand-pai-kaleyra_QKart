  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        ...
        minRows={4}
        onChange={(ev) => {
          handleNewAddress({ ...newAddress, value: ev.target.value })
        }}
      ...
      <Stack direction="row" my="1rem">
        <Button
          ...
          onClick={() => {
            ...
            handleNewAddress({ ...newAddress, isAddingNewAddress: false, value: "" })
          }}
        >
          Add
        </Button>
...
const Checkout = () => {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    if (validateRequest(items, addresses)) {
      toDeduct = getTotalCartValue(items)
      ...
      if (response && response.status === 200) {
        localStorage.setItem('balance', parseInt(localStorage.getItem('balance')) - parseInt(toDeduct))
        history.push('/thanks')
      }
    }
    ...
    <>
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              {
                addresses.all.length > 0 ? (
                  addresses.all.map((item, index) => {
                    return <Box key={index} className={`address-item ${item._id === addresses.selected ? 'selected' : 'not-selected'}`} onClick={() => {
            ...
            <Button
              ...
              variant="contained"
              onClick={() => {
                performCheckout(token, items, addresses).then((res) => {
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
          ...
          <Box className="cart">
            <Typography variant="h5" my="0.5rem" p={2} fontWeight={500}>
              Order Details
            </Typography>
            <Box
              display="flex"
